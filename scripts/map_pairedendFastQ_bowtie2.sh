...
do 
 ...
 mappingLogFile=${mappingLogsDir}/${fastQfile}_bowtie2_mapping.log
 ...
 echo "bowtie2 -x ${bt2index} -p 8 --very-sensitive -1 ${fq1} -2 ${fq2} -S ${bamdir}/${fastQfile}.sam  2>&1 | tee ${mappingLogFile}" > "${scriptFile}"
